...
import { companyService, eventService } from '../../services/api';
...
const AdminCompanyApprovals = () => {
  ...
  const fetchApprovals = async () => {
    ...
    setError('');
    try {
      const [companyData, requestData] = await Promise.all([
        companyService.getPendingCompanies(),
        eventService.getFeaturedRequests(),
      ]);
      setCompanies((companyData.companies || []).map((company) => ({
        ...company,
      ...
      setFeaturedRequests(requestData || []);
    } catch (err) {
      console.error('Failed to load approvals:', err);
      setError(err.response?.data?.message || 'Failed to load pending approvals from the database.');
      setCompanies([]);
      setFeaturedRequests([]);
    } finally {
      setLoading(false);
  ...
  const handleFeaturedReview = async (id, title, approved) => {
    const action = approved ? 'approve' : 'reject';
    if (!window.confirm(`Are you sure you want to ${action} the featured request for "${title}"?`)) return;
    try {
      await eventService.reviewFeaturedRequest(id, approved);
      fetchApprovals();
    } catch (err) {
      alert(err.response?.data?.message || `Failed to ${action} featured request`);
    }
  };
  ...
  return (
    <div className="max-w-container-max mx-auto space-y-8 relative">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4"><div><h2 className="text-3xl font-bold text-on-surface">Pending Approvals</h2><p className="text-sm text-on-surface-variant mt-1">Review company registrations and featured event requests.</p></div></div>
      {error && <div className="p-4 bg-error-container/30 text-error rounded-xl">{error}</div>}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-white p-6 rounded-xl border border-border-subtle shadow-sm flex items-center gap-4"><div className="p-3 bg-primary-container/10 rounded-lg text-primary shrink-0"><span className="material-symbols-outlined">pending_actions</span></div><div><p className="text-xs text-on-surface-variant font-semibold">Company Pending</p><h4 className="text-2xl font-bold font-mono">{companies.length}</h4></div></div>
        <div className="bg-white p-6 rounded-xl border border-border-subtle shadow-sm flex items-center gap-4"><div className="p-3 bg-accent-yellow/10 rounded-lg text-accent-yellow shrink-0"><span className="material-symbols-outlined">stars</span></div><div><p className="text-xs text-on-surface-variant font-semibold">Featured Requests</p><h4 className="text-2xl font-bold font-mono">{featuredRequests.length}</h4></div></div>
      </div>

      <section className="bg-white rounded-xl border border-border-subtle shadow-sm overflow-hidden">
        <div className="px-6 py-4 border-b border-border-subtle"><h3 className="font-bold text-on-surface">Company Registrations</h3></div>
      ...
      <section className="bg-white rounded-xl border border-border-subtle shadow-sm overflow-hidden">
        <div className="px-6 py-4 border-b border-border-subtle"><h3 className="font-bold text-on-surface">Featured Event Requests</h3></div>
        {loading ? <div className="flex items-center justify-center p-12"><div className="animate-spin rounded-full h-10 w-10 border-t-2 border-b-2 border-primary"></div></div> : <div className="overflow-x-auto"><table className="w-full text-left border-collapse"><thead><tr className="bg-surface-gray border-b border-border-subtle text-xs font-bold text-on-surface-variant uppercase tracking-wider"><th className="px-6 py-4">Event</th><th className="px-6 py-4">Planner Company</th><th className="px-6 py-4">Category</th><th className="px-6 py-4">Date</th><th className="px-6 py-4 text-right">Actions</th></tr></thead><tbody className="divide-y divide-border-subtle text-sm text-on-surface">{featuredRequests.length === 0 ? <tr><td colSpan="5" className="px-6 py-12 text-center text-on-surface-variant">No featured event requests are pending.</td></tr> : featuredRequests.map((event) => <tr key={event.id} className="hover:bg-surface-container-low transition-colors"><td className="px-6 py-4"><div className="flex items-center gap-3"><div className="w-12 h-12 rounded-lg overflow-hidden bg-surface-container shrink-0">{event.image ? <img src={event.image} alt={event.title} className="w-full h-full object-cover" /> : null}</div><div><p className="font-semibold text-on-surface">{event.title}</p><p className="text-xs text-on-surface-variant">{event.location}</p></div></div></td><td className="px-6 py-4 font-medium text-on-surface-variant">{event.company?.companyName || 'Unknown'}</td><td className="px-6 py-4"><span className="px-2.5 py-1 rounded-full text-xs font-semibold bg-secondary-container text-on-secondary-container">{event.category}</span></td><td className="px-6 py-4 text-on-surface-variant">{new Date(event.date).toLocaleDateString()}</td><td className="px-6 py-4 text-right"><div className="flex items-center justify-end gap-2"><button onClick={() => handleFeaturedReview(event.id, event.title, true)} className="px-3 py-1.5 rounded-lg bg-secondary text-on-secondary text-xs font-semibold">Approve</button><button onClick={() => handleFeaturedReview(event.id, event.title, false)} className="px-3 py-1.5 rounded-lg border border-error text-error text-xs font-semibold">Reject</button></div></td></tr>)}</tbody></table></div>}
      </section>
